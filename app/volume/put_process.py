...
class PutProcess:
    ...
    def notify_record_to_master(self, env) -> bool:
        ...
        if Constants.SET_TEMPORAL in query_params:
            temporal = query_params[Constants.SET_TEMPORAL][0]
        master_url = self.utils.extract_url_data(
            query_params['url'][0])
        # Prod url
        host_url = master_url['host']
        # TODO: MAC support
        if Constants.DEBUG_WIN:
        ...
        req = CoreRequest(
            host_url,
            master_url['port'],
            Constants.ADD_RECORD
        )
        req.set_type(ReqType.POST)
        req.add_header(
            {
                Constants.PASSPORT: self.utils.encode_base_64(key)
            }
        )
        ...
        if regex_result:
            try:
                ...
                if self.notify_record_to_master(env):
                    self.messenger.code_type = Constants.KEY_ADDED
